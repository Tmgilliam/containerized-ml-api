...
class ShadowComparator:
    ...
    def is_shadow_ready_for_promotion(self) -> tuple[bool, list[str]]:
        ...
        if result.prediction_agreement_rate < self.agreement_threshold:
            issues.append(
                f"Agreement rate too low: {result.prediction_agreement_rate:.2%} < {self.agreement_threshold:.2%}"
            )
        
        if result.mean_probability_difference > self.probability_diff_threshold:
            issues.append(
                f"Mean probability difference too high: {result.mean_probability_difference:.4f} > {self.probability_diff_threshold}"
            )
